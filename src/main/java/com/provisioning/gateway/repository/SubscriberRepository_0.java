...
public interface SubscriberRepository_0 extends CrudRepository<Subscriber_0, String>,MainRepository {
	...
	@Transactional
	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE subscribers_1  SET status = :status,hlr_deactivation_date = now(),unsub_date= now() WHERE a_party =:a_party", nativeQuery = true)
	public int removeVipCallsubscriber(@Param("a_party") String a_party, @Param("status") int status);

	@Transactional
	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE subscribers_1  SET status = :status WHERE a_party =:a_party", nativeQuery = true)
