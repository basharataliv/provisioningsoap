...
public interface SubscriberRepository_6 extends CrudRepository<Subscriber_6, String>,MainRepository {
	...
	@Transactional
	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE subscribers_6  SET status = :status,hlractivation_date = now(),sub_date= now() WHERE a_party =:a_party", nativeQuery = true)
	public int addVipCallsubscriber(@Param("a_party") String a_party, @Param("status") int status);

	// this method used to update the add vip subscription
	@Transactional
	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE subscribers_6  SET status = :status,hlr_deactivation_date = now(),unsub_date= now() WHERE a_party =:a_party", nativeQuery = true)
	public int removeVipCallsubscriber(@Param("a_party") String a_party, @Param("status") int status);

	@Transactional
	@Modifying(clearAutomatically = true)
	@Query(value = "UPDATE subscribers_6  SET status = :status WHERE a_party =:a_party", nativeQuery = true)
